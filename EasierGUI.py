...
if if_gpu_ok == True and len(gpu_infos) > 0:
    ...
    gpu_info = i18n("É uma pena que você não tenha uma placa de vídeo funcionando para suportar seu treinamento")
...
def change_version19(sr2, if_f0_3, version19):
    ...
    if_pretrained_discriminator_exist = os.access("pretrained%s/%sD%s.pth" % (path_str, f0_str, sr2), os.F_OK)
    if (if_pretrained_generator_exist == False):
        print("pretrained%s/%sG%s.pth" % (path_str, f0_str, sr2), "não existe, não usará o modelo pré-treinado")
    if (if_pretrained_discriminator_exist == False):
        print("pretrained%s/%sD%s.pth" % (path_str, f0_str, sr2), "não existe, não usará o modelo pré-treinado")
    return (
        ("pretrained%s/%sG%s.pth" % (path_str, f0_str, sr2)) if if_pretrained_generator_exist else "",
        ("pretrained%s/%sD%s.pth" % (path_str, f0_str, sr2)) if if_pretrained_discriminator_exist else "",
    )
...
def change_f0(if_f0_3, sr2, version19):  # f0method8,pretrained_G14,pretrained_D15
    ...
    if (if_pretrained_generator_exist == False):
        print("pretrained%s/f0G%s.pth" % (path_str, sr2), "não existe, não usará o modelo pré-treinado")
    if (if_pretrained_discriminator_exist == False):
        print("pretrained%s/f0D%s.pth" % (path_str, sr2), "não existe, não usará o modelo pré-treinado")
    if if_f0_3:
        return (
            {"visible": True, "__type__": "update"},
...
def click_train(
    ...
    p.wait()
    return "Após o término do treinamento, você pode verificar o log de treinamento do console ou train.log na pasta log"
...
def train_index(exp_dir1, version19):
    ...
    if os.path.exists(feature_dir) == False:
        return "Execute a extração de recursos primeiro!"
    ...
    infos.append(
        "Índice criado com sucesso，added_IVF%s_Flat_nprobe_%s_%s_%s.index"
...
def train1key(
    ...
    yield get_info_str(i18n("step1:processando dados"))
    ...
    if if_f0_3:
        yield get_info_str("step2a:extraindo piche")
        ...
        yield get_info_str(i18n("step2a:Não há necessidade de extrair pitch"))
    #######step2b:提取特征
    yield get_info_str(i18n("step2b:Extraindo recursos"))
    gpus = gpus16.split("-")
    leng = len(gpus)
    ps = []
    for idx, n_g in enumerate(gpus):
    ...
    with open(extract_f0_feature_log_path, "r") as f:
        print(f.read())
    #######step3a:训练模型
    yield get_info_str(i18n("step3a:modelo de treinamento"))
    # 生成filelist
    if if_f0_3:
    ...
    p = Popen(cmd, shell=True, cwd=now_dir)
    p.wait()
    yield get_info_str(i18n("Após o término do treinamento, você pode verificar o log de treinamento do console ou train.log na log"))
    #######step3b:训练索引
    npys = []
    listdir_res = list(os.listdir(feature_dir))
    for name in sorted(listdir_res):
    ...
    yield get_info_str("adicionando índice")
    batch_size_add = 8192
    for i in range(0, big_npy.shape[0], batch_size_add):
        index.add(big_npy[i : i + batch_size_add])
    faiss.write_index(
    ...
    yield get_info_str(i18n("Todo o processo acabou！"))
...
def cli_infer(com):
    ...
    print(vc_data)
    print("Mangio-RVC-Fork Infer-CLI: Fazendo inferência...")
    ...
    if "Success." in conversion_data[0]:
        print("Mangio-RVC-Fork Infer-CLI: A inferência foi bem-sucedida. Escrevendo para %s/%s..." % ('audio-outputs', output_file_name))
        wavfile.write('%s/%s' % ('audio-outputs', output_file_name), conversion_data[1][0], conversion_data[1][1])
        print("Mangio-RVC-Fork Infer-CLI: Finalizado! Saída salva em %s/%s" % ('audio-outputs', output_file_name))
    else:
        print("Mangio-RVC-Fork Infer-CLI: A inferência falhou. Aqui está o rastreamento: ")
...
def cli_extract_feature(com):
    ...
    print("Mangio-RVC-CLI: O recurso de extração tem pitch:" + str(has_pitch_guidance))
    print("Mangio-RVC-CLI: Extrair versão do recurso: " + str(version))
...
def cli_train_feature(com):
    ...
    model_name = com[0]
    version = com[1]
    print("Mangio-RVC-Fork Treinando as caracteristica do Index-CLI: Treinamento... Por favor, aguarde")
    generator = train_index(
        model_name,
        version
    )
    execute_generator_function(generator)
    print("Mangio-RVC-Fork Train Feature Index-CLI: Feito!")
...
def cli_extract_model(com):
    ...
    if extract_small_model_process == "Sucedida.":
        print("Mangio-RVC-Fork Extract Small Model: Sucedida!")
    else:
        print(str(extract_small_model_process))        
        print("Mangio-RVC-Fork Extract Small Model: Fracassado!")
...
def print_page_details():
    if cli_current_page == "HOME":
        ...
        print('Exemplo: logs/mi-test/G_168000.pth MyModel 40k 1 "Criado por Cole Mangio" v2')
    print("")
...
def execute_command(com):
    if com == "go home":
        ...
        if com[:3] == "go ":
            print("página '%s' não existe!" % com[3:])
            return 0
...
def cli_navigation_loop():
    while True:
        print("Você está atualmente em '%s':" % cli_current_page)
        print_page_details()
...
if(config.is_cli):
    ...
    print("Bem-vindo à versão CLI do RVC. Leia a documentação em https://github.com/Mangio621/Mangio-RVC-Fork (README.MD) para entender como usar este aplicativo.\n")
    cli_navigation_loop()
...
def zip_downloader(model):
    if not os.path.exists(f'./weights/{model}.pth'):
        return {"__type__": "update"}, f'Certifique-se de que o nome da voz esteja correto. Não consegui encontrar {model}.pth'
    index_found = False
...
with gr.Blocks(theme=gr.themes.Base()) as app:
    with gr.Tabs():
        with gr.TabItem("Inferência"):
            gr.HTML("<h1> Easy GUI v2 (rejekts) - adaptado para Mangio-RVC-Fork 💻 Traduzido por: Rafael Godoy | AI HUB BRASIL</h1>")
            ...
            # Other RVC stuff
            with gr.Row():
                sid0 = gr.Dropdown(label="1.Escolha o seu Modelo.", choices=sorted(names), value=check_for_name())
                refresh_button = gr.Button("Atualizar", variant="primary")
                if check_for_name() != '':
                    get_vc(sorted(names)[0])
                vc_transform0 = gr.Number(label="Opcional: você pode alterar o tom aqui ou deixá-lo em 0. (inteiro, número de semitons, oitava sustenido 12, oitava bemol -12)", value=0)
                #clean_button = gr.Button(i18n("卸载音色省显存"), variant="primary")
                spk_item = gr.Slider(
                    minimum=0,
                    maximum=2333,
                    step=1,
                    label=i18n("Por favor, selecione o ID do alto-falante"),
                    value=0,
                    visible=False,
                    interactive=True,
                )
            ...
            with gr.Row():
                with gr.Column():
                    with gr.Row():
                        dropbox = gr.File(label="Solte seu áudio aqui e aperte o botão Atualizar.")
                    with gr.Row():
                        record_button=gr.Audio(source="microphone", label="Ou Grave áudio.", type="filepath")
                    with gr.Row():
                        input_audio0 = gr.Dropdown(
                            label="2.Escolha o seu áudio.",
                            value="./audios/algum-audio.mp3",
                            choices=audio_files
                            )
                    ...
                    with gr.Row():
                        with gr.Accordion('Texto para fala', open=False):
                            with gr.Column():
                                lang = gr.Radio(label='Chinês e japonês não funcionam com o ElevenLabs atualmente.',choices=['en','es','fr','pt','zh-CN','de','hi','ja'], value='en')
                                api_box = gr.Textbox(label="Digite sua chave de API para ElevenLabs ou deixe em branco para usar o GoogleTTS", value='')
                                elevenid=gr.Dropdown(label="Voz:", choices=eleven_voices)
                            with gr.Column():
                                tfs = gr.Textbox(label="Insira seu texto", interactive=True, value="Isto é um teste.")
                                tts_button = gr.Button(value="Falar")
                                tts_button.click(fn=elevenTTS, inputs=[api_box,tfs, elevenid, lang], outputs=[record_button, input_audio0])
                    with gr.Row():
                        with gr.Accordion('Wav2Lip', open=False):
                            with gr.Row():
                                size = gr.Radio(label='Resolução:',choices=['Média','Alto'])
                                face = gr.UploadButton("Carregar um personagem",type='file')
                ...
                with gr.Column():
                    with gr.Accordion("Configurações do Index", open=False):
                        file_index1 = gr.Dropdown(
                            label="3. Caminho para o arquivo Added.index (caso não tenha sido encontrado automaticamente).",
                            choices=get_indexes(),
                            value=get_index(),
                            interactive=True,
                            )
                    ...
                    vc_output2 = gr.Audio(label="Áudio de saída (clique nos três pontos no canto direito para fazer o download)",type='filepath')
                    animate_button.click(fn=mouth, inputs=[size, face, vc_output2, faces], outputs=[animation, preview])
                    with gr.Accordion("Configurações avançadas", open=False):
                        f0method0 = gr.Radio(
                            label="Opcional: Altere o Algoritmo de Extração de Pitch.",
                        ...
                        crepe_hop_length = gr.Slider(
                            ...
                            maximum=512,
                            step=1,
                            label="Mangio-Crepe Hop Length. Números mais altos reduzirão a chance de mudanças extremas de tom, mas números mais baixos aumentarão a precisão.",
                            value=120,
                            interactive=True
                            )
                        filter_radius0 = gr.Slider(
                            minimum=0,
                            maximum=7,
                            label=i18n(">=3, use o filtro mediano para o resultado do reconhecimento do tom da colheita, e o valor é o raio do filtro, que pode enfraquecer o mudo"),
                            value=3,
                            step=1,
                            interactive=True,
                            )
                        ...
                        rms_mix_rate0 = gr.Slider(
                            minimum=0,
                            maximum=1,
                            label=i18n("O envelope de volume da fonte de entrada substitui a taxa de fusão do envelope de volume de saída, quanto mais próximo de 1, mais o envelope de saída é usado"),
                            value=0.21,
                            interactive=True,
                            )
                        protect0 = gr.Slider(
                            minimum=0,
                            maximum=0.5,
                            label=i18n("Proteja consoantes sem voz e sons respiratórios, evite artefatos como quebra de som eletrônico e desligue-o quando estiver cheio de 0,5. Diminua-o para aumentar a proteção, mas pode reduzir o efeito de indexação"),
                            value=0.33,
                            step=0.01,
                            interactive=True,
                            )
            with gr.Row():
                ...
                f0_file = gr.File(label=i18n("Arquivo de curva F0, opcional, um passo por linha, em vez do padrão F0 e altos e baixos"), visible=False)
            ...
            with gr.Accordion("Conversão em Massa",open=False):
                with gr.Row():
                    with gr.Column():
                        vc_transform1 = gr.Number(
                            label=i18n("Transpor (inteiro, número de semitons, oitava sustenido 12, oitava bemol -12)"), value=0
                        )
                        opt_input = gr.Textbox(label=i18n("Especifique a pasta de saída:"), value="opt")
                        f0method1 = gr.Radio(
                            label=i18n(
                                "Selecione o algoritmo de extração de tom, o canto de entrada pode usar pm para acelerar, a colheita de graves é boa, mas extremamente lenta, o efeito crepe é bom, mas requer GPU"
                            ),
                    ...
                    with gr.Column():
                        file_index3 = gr.Textbox(
                            label=i18n("Caminho do arquivo da biblioteca de recuperação de recursos, se estiver vazio, use o resultado da seleção suspensa"),
                            value="",
                            interactive=True,
                        )
                    ...
                    with gr.Row():
                        format1 = gr.Radio(
                            label=i18n("Qual formato de arquivo você prefere?"),
                            choices=["wav", "flac", "mp3", "m4a"],
                            value="flac",
                            interactive=True,
                        )
                        but1 = gr.Button(i18n("Converter"), variant="primary")
                        vc_output3 = gr.Textbox(label=i18n("Informações de saída"))
                    but1.click(
                        vc_multi,
                        [
                            spk_item,
        ...
        with gr.TabItem("Baixe um modelo"):
            with gr.Row():
                url=gr.Textbox(label="Insira a URL para o modelo:")
            with gr.Row():
                model = gr.Textbox(label="Dê um nome ao seu modelo:")
            ...
            with gr.Row():
                ...
                ❤️ Se você gosta do EasyGUI, ajude-me a mantê-lo.❤️ 
                https://paypal.me/lesantillan
                ❤️ Se você gosta da tradução, ajude-me a mantê-la.❤️
                https://nubank.com.br/pagar/1ls6a4/0QpSSbWBSq
                """
                )
        ...
        with gr.TabItem("Treinar", visible=False):
            with gr.Row():
                with gr.Column():
                    exp_dir1 = gr.Textbox(label="Nome da voz:", value="Meu-modelo-de-voz")
                    sr2 = gr.Radio(
                    ...
                    if_f0_3 = gr.Radio(
                        label=i18n("O modelo tem orientação de tom (o canto deve, a voz não)"),
                        choices=[True, False],
                        value=True,
                        interactive=True,
                        visible=False
                    ...
                    np7 = gr.Slider(
                        ...
                        step=1,
                        label="# de CPUs para usar (Deixe-o, a menos que você saiba o que está fazendo!)",
                        value=config.n_cpu,
                        interactive=True,
                        visible=False
                    )
                    trainset_dir4 = gr.Textbox(label="Caminho para o seu conjunto de dados/dataset (áudios, não zip):", value="./dataset")
                    easy_uploader = gr.Files(label='Ou Solte seus áudios aqui. Eles serão carregados no caminho do conjunto de dados acima.',file_types=['audio'])
                    but1 = gr.Button("1. Processar o conjunto de dados(dataset)", variant="primary")
                    info1 = gr.Textbox(label="Status (espere até aparecer 'Finalizado pré-processamento'):", value="")
                ...
                with gr.Column():
                    spk_id5 = gr.Slider(
                        ...
                        maximum=4,
                        step=1,
                        label=i18n("Especificar a identificação do orador"),
                        value=0,
                        interactive=True,
                        visible=False
                    )
                    with gr.Accordion('Configurações da GPU', open=False, visible=False):
                        gpus6 = gr.Textbox(
                            label=i18n("Introduzir os números dos cartões utilizados separados por -, por exemplo, 0-1-2 utilizando o cartão 0 e os cartões 1 e 2"),
                            value=gpus,
                            interactive=True,
                            visible=False
                        )
                        gpu_info9 = gr.Textbox(label=i18n("Informações sobre a placa gráfica"), value=gpu_info)
                    f0method8 = gr.Radio(
                        label=i18n(
                            "Selecionar o algoritmo de extração de tom: a voz de entrada pode ser acelerada com pm, a voz de alta qualidade mas CPU pobre pode ser acelerada com dio, a qualidade da colheita é melhor mas lenta"
                        ),
                    ...
                    but2 = gr.Button("2. Extrair Pitch", variant="primary")
                    info2 = gr.Textbox(label="Status (Verifique no CMD):", value="", max_lines=8)
                    but2.click(
                            extract_f0_feature,
                ...
                with gr.Row():      
                    with gr.Column():
                        total_epoch11 = gr.Slider(
                            ...
                            maximum=10000,
                            step=10,
                            label="Número total de ciclos(epoch) de treino (se escolher um valor alto demais, o seu modelo parecerá terrivelmente sobretreinado):",
                            value=250,
                            interactive=True,
                        )
                        but3 = gr.Button("3.Treinar Modelo", variant="primary")
                        but4 = gr.Button("4. Treinar Index", variant="primary")
                        info3 = gr.Textbox(label="Status (Verfique no CMD):", value="", max_lines=10)
                        with gr.Accordion("Preferências de treinamento (você pode deixá-las como estão)", open=False):
                            ...
                            with gr.Column():
                                save_epoch10 = gr.Slider(
                                    ...
                                    maximum=100,
                                    step=5,
                                    label="Faça backup a cada # de Epoch:",
                                    value=25,
                                    interactive=True,
                                )
                                batch_size12 = gr.Slider(
                                    ...
                                    maximum=40,
                                    step=1,
                                    label="Batch Size - Tamanho do lote (DEIXE-O, a menos que saiba o que está fazendo!):",
                                    value=default_batch_size,
                                    interactive=True,
                                )
                                if_save_latest13 = gr.Radio(
                                    label=i18n("Só deve salvar apenas o arquivo ckpt mais recente para economizar espaço em disco"),
                                ...
                                if_cache_gpu17 = gr.Radio(
                                    label=i18n(
                                        "Se deve armazenar em cache todos os conjuntos de treinamento na memória de vídeo. Pequenos dados com menos de 10 minutos podem ser armazenados em cache para acelerar o treinamento, e um cache de dados grande irá explodir a memória de vídeo e não aumentar muito a velocidade"
                                    ),
                                    choices=[i18n("Sim"), i18n("Não")],
                                    value=i18n("Sim"),
                                    interactive=True,
                                ...
                                if_save_every_weights18 = gr.Radio(
                                    label=i18n("Salve um pequeno modelo final na pasta 'weights' em cada ponto de salvamento:"),
                                    choices=[i18n("Sim"), i18n("Não")],
                                    value=i18n("Sim"),
                                    interactive=True,
                                )
                        zip_model = gr.Button('5.Baixar Modelo')
                        zipped_model = gr.Files(label='Seu modelo e arquivo de índice podem ser baixados aqui:')
                        zip_model.click(fn=zip_downloader, inputs=[exp_dir1], outputs=[zipped_model, info3])
            with gr.Group():
                with gr.Accordion("Locais do modelo básico:", open=False, visible=False):
                    pretrained_G14 = gr.Textbox(
                        label=i18n("Carregue o caminho G do modelo inferior pré-treinado"),
                        value="pretrained_v2/f0G40k.pth",
                        interactive=True,
                    )
                    pretrained_D15 = gr.Textbox(
                        label=i18n("Carregue o caminho D do modelo base pré-treinado"),
                        value="pretrained_v2/f0D40k.pth",
                        interactive=True,
                    )
                    gpus16 = gr.Textbox(
                        label=i18n("Insira os números dos cartões usados ​​separados por -, por exemplo 0-1-2 use o cartão 0 e o cartão 1 e o cartão 2"),
                        value=gpus,
                        interactive=True,
                    )
                sr2.change(
                ...
                but5 = gr.Button(i18n("Um treinamento chave"), variant="primary", visible=False)
                but3.click(
                    click_train,
                    [
                        exp_dir1,
            ...
            try:
                if tab_faq == "perguntas frequentes":
    ...
    def save_preset(preset_name,sid0,vc_transform,input_audio,f0method,crepe_hop_length,filter_radius,file_index1,file_index2,index_rate,resample_sr,rms_mix_rate,protect,f0_file):
        ...
        with open('../inference-presets.json', 'w') as file:
            json.dump(data, file)
            file.flush()
        print("Predefinição %s salva em inference-presets.json!" % preset_name)


    def on_preset_changed(preset_name):
        print("Predefinição alterada para %s!" % preset_name)
        data = None
        with open('../inference-presets.json', 'r') as file:
            data = json.load(file)

        print("Procurando por " + preset_name)
        returning_preset = None
        for preset in data['presets']:
            if(preset['name'] == preset_name):
                print("Encontrou uma predefinição")
